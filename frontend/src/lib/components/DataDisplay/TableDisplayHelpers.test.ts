...
describe("filterItems: should return items in given columns ", () => {
	...
	it("should return all items when the search term is empty", () => {
		const searchTerm = "";
		const result = filterItemsDefault(data, searchTerm, searchableColumns);
	...
	it("should return items case sensitively", () => {
		...
		const resultUpper = filterItemsDefault(
			data,
			searchTermUpper,
			searchableColumns,
		);
		const resultLower = filterItemsDefault(
			data,
			searchTermLower,
			searchableColumns,
		);
	...
	it("should return nothing if there are no searchable columns", () => {
		const otherSearchableColumns = [] as string[];
		const searchTerm = "Jane";
		const result = filterItemsDefault(data, searchTerm, otherSearchableColumns);
		expect(result).toEqual([]);
	});
});
...
describe("makePlaceholderText: should return a placeholder text based on the number of searchable columns", () => {
	...
	it("should return colname when only one searchable columns", () => {
		...
		const result = makePlaceholderTextDefault(data, searchableColumns);
