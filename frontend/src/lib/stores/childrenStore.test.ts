...
describe("normal functionality", () => {
	...
	it("should add child successfully", async () => {
		...
		expect(children.get()["alpha"]["childC"].childData).toEqual(
			mockChildData3.childData,
		);
	});
	...
	it("should add child observationdata successfully", async () => {
		...
		await children.addChildData("alpha", "childC", mockChildData3.childData);
		await children.addChildObservation(
			"alpha",
			"childC",
	...
	it("should throw when removing with nonexistant user or nonexisting child key", async () => {
		...
		try {
			...
			expect((error as Error).message).toBe(
				"Child token notthere not found for user token alpha",
			);
		}
	});
	...
	it("should fetch observation data", async () => {
		reset();
		expect(await children.fetchObservationData("alpha", "childfoo")).toEqual(
			mockObservationData,
		);
	});
	...
	it("cannot fetch from unknown keys", async () => {
		...
		try {
			...
			expect((error as Error).message).toBe(
				"No such user in the childrenstore",
			);
		}
		...
		try {
			...
			expect((error as Error).message).toBe(
				"No such child in the childrenstore for user alpha",
			);
		}
	});
