        ...
        pd.DataFrame(
            {
                "VAR": ["GM_01", "HM_01"],
                "LABEL": [
                    "Grobmotorik: Test Milestone 1",
                    "Handmotorik: Test Milestone 2",
                ],
...
def import_manager(temp_db_path, import_paths):
    ...
    manager.hardcoded_id_map = {"TEST01": 1, "TEST02": 2}
...
class TestImportManager:
    ...
    def test_get_question_filled_in_to_parent(self, import_manager):
        ...
        assert not import_manager.get_question_filled_in_to_parent(
            questions_df, "TEST01"
        )
    ...
    def test_derive_milestone_group(self, import_manager):
        ...
        assert (
            import_manager.derive_milestone_group_from_milestone_string_id("GM_01")
            == "2"
        )
        assert (
            import_manager.derive_milestone_group_from_milestone_string_id("HM_02")
            == "1"
        )
        assert (
            import_manager.derive_milestone_group_from_milestone_string_id("SE_03")
            == "5"
        )

        # Test invalid milestone group
        assert (
            import_manager.derive_milestone_group_from_milestone_string_id("XX_01")
            is None
        )
        assert (
            import_manager.derive_milestone_group_from_milestone_string_id("G") is None
        )
    ...
    async def test_import_milestones_metadata(self, mock_select, import_manager):
        ...
        import_manager.update_milestone_with_name_property = MagicMock(
            return_value=True
        )
