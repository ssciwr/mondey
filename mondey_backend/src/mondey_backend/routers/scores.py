...
def compute_feedback_milestone_group(
    ...
    stddev = stats.stddev
    if stddev <= 0:
        # every session in the statistics population had the same score for this
        # milestone group and age, so we cannot say how far from typical this one is
        logger.debug(f"  score: {score}, mean: {mean}, stddev: {stddev}: no variation")
        return TrafficLight.invalid.value
