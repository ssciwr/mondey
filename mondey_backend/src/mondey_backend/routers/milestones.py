...
def create_router() -> APIRouter:
    ...
    @router.get(
        "/milestone-age-groups/{milestone_age_group_id}",
        response_model=MilestoneAgeGroupPublic,
    )
    def get_milestone_age_group(session: SessionDep, milestone_age_group_id: int):
        milestone_age_group = session.get(MilestoneAgeGroup, milestone_age_group_id)
        if not milestone_age_group:
            raise HTTPException(status_code=404, detail="milestone_age_group not found")
        return milestone_age_group
