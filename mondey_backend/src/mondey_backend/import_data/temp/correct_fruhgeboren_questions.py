...
def transform_birth_terms(dry_run=False):
    ...
    with next(get_session()) as session:
        ...
        if not birth_question or not incubator_question:
            # Get the highest existing question ID to ensure we use new unique IDs
            max_id_result = session.exec(
                select(ChildQuestion).order_by(ChildQuestion.id.desc())
            ).first()
            max_id = max_id_result.id if max_id_result else 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Transform birth terms data")
    parser.add_argument(
        "--dry-run",
        "-d",
        action="store_true",
    ...
    transform_birth_terms(dry_run=args.dry_run)
